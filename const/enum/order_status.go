...
const (
	PrefixOrderStatus PrefixType = "order_status" // 订单状态前缀
)
...
func NewOrderStatus(code int, desc string) *OrderStatus {
	...
	Set(PrefixOrderStatus, code, desc)
	return status
}

// ParseOrderStatus 解析订单状态
func ParseOrderStatus(code int) (*OrderStatus, error) {
	if desc, ok := Get(PrefixOrderStatus, code); ok {
		return &OrderStatus{
			BaseEnum: BaseEnum{
				code: code,
				desc: desc,
...
func (s *OrderStatus) Scan(value interface{}) error {
	return s.BaseEnum.Scan(value, GetAll(PrefixOrderStatus))
}

// UnmarshalJSON 实现 json.Unmarshaler 接口
func (s *OrderStatus) UnmarshalJSON(data []byte) error {
	return s.BaseEnum.UnmarshalJSON(data, GetAll(PrefixOrderStatus))
}
